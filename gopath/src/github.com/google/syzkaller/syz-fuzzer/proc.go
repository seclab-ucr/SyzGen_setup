...
func (proc *Proc) loop() {
	generatePeriod := 100
	if proc.fuzzer.config.Flags&(ipc.FlagSignal|ipc.FlagEnableHook) == 0 {
...
func (proc *Proc) execute(execOpts *ipc.ExecOpts, p *prog.Prog, flags ProgTypes, stat Stat) *ipc.ProgInfo {
	...
	if len(info.Hooks.Signal) > 0 {
		// Add it to the corpus
		data := p.Serialize()
		sig := hash.Hash(data)

		log.Logf(2, "added new input (%d) for %v to corpus:\n%s", len(info.Hooks.Signal), "hook", data)

		inputSignal := signal.FromRaw(info.Hooks.Signal, 0)

		proc.fuzzer.sendInputToManager(rpctype.RPCInput{
			Call:   ".hook",
			Prog:   data,
			Signal: inputSignal.Serialize(),
			Cover:  info.Hooks.Signal,
		})
...
func (proc *Proc) executeRaw(opts *ipc.ExecOpts, p *prog.Prog, stat Stat) *ipc.ProgInfo {
	...
	for try := 0; ; try++ {
		...
		if err != nil {
			if try > 10 {
				log.Logf(0, "executor %v failed %v times:\n%v", proc.pid, try, err)
				break
			}
