...
func OpenDarwinConsole(debug bool, sshkey, sshuser, addr, command string) (rc io.ReadCloser, err error) {
	rpipe, wpipe, err := osutil.LongPipe()
	if err != nil {
		return nil, err
	}
	sshArgs := SSHArgs(debug, sshkey, 22)
	args := []string{"ssh"}
	args = append(args, sshArgs...)
	args = append(args, sshuser+"@"+addr, command)
