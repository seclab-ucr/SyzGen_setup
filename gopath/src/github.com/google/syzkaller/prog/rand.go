...
func (r *randGen) createResource(s *state, res *ResourceType, dir Dir) (arg Arg, calls []*Call) {
	...
	if len(r.target.resourceMap) != 0 && r.oneOf(1000) && !r.generate{
	...
	if r.generate {
		for _, ctor := range res.Desc.Ctors {
			if ctor.Precise {
				meta := r.target.Syscalls[ctor.Call]
...
func (a *ResourceType) generate(r *randGen, s *state, dir Dir) (arg Arg, calls []*Call) {
	if r.generate {
		// Reuse the nearest resource
		arg = r.nearestResource(s, a, dir)
		if arg != nil {
			return
		}
...
func (r *randGen) nearestResource(s *state, res *ResourceType, dir Dir) Arg {
	for _, res1 := range s.resources {
		name1 := res1[0].Type().Name()
		if r.target.isCompatibleResource(res.Desc.Name, name1) {
			return MakeResultArg(res, dir, res1[len(res1)-1], 0)
		}
	}
	return nil
}
