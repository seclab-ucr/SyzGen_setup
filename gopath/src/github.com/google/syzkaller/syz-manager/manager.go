...
var (
	...
	flagBpCov  = flag.Bool("bpcov", false, "using breakpoints to get block coverage")
	flagBpFile = flag.String("kcov", "", "path to the kcov config file")
	flagCorpus = flag.Bool("corpus", false, "generate concise corpus")
)
...
type Manager struct {
	...
	// Use breakpoints to collect coverage
	useBreakpoint  bool
}
...
func main() {
	...
	if *flagBpCov && *flagBpFile == "" {
		log.Fatalf("kcov is empty")
	}
...
func (mgr *Manager) runInstance(index int) (*Crash, error) {
	...
	if *flagBpCov {
		log.Logf(0, "copy kcov file %v", *flagBpFile)
		if _, err := inst.Copy2VM(*flagBpFile, "/tmp/kcov"); err != nil {
			return nil, fmt.Errorf("failed to copy kcov: %v", err)
		}
		log.Logf(1, "load kcov driver")
		load_cmd := instance.DriverCmd(mgr.cfg.TargetOS)
		if _, err := inst.RunCmd(load_cmd); err != nil {
			return nil, fmt.Errorf("failed to load driver: %v", err)
		}
	}
	// Setup debugger to monitor coverage (Note only one machine unless you specify the config)
	// var debugger *exec.Cmd = nil
	// if mgr.useBreakpoint {
	// 	log.Logf(0, "sh coverage.sh")
	// 	debugger = osutil.Command("sh", "coverage.sh")
	// 	debugger.Dir = "/Users/CwT/git/angr"  // FIXME
	// 	if err := debugger.Start(); err != nil {
	// 		return nil, fmt.Errorf("failed to run debugger: %v", err)
	// 	}
	// }
	...
	cmd := instance.FuzzerCmd(fuzzerBin, executorCmd, fmt.Sprintf("vm-%v", index),
		...
		mgr.cfg.Cover, mgr.useBreakpoint, *flagDebug, false, false)
	...
	// TODO: check if it is due to block of breakpoint
	// Has debugger finished??
	// if rep != nil && mgr.useBreakpoint && debugger != nil {
	...
	// 		if err := debugger.Process.Kill(); err != nil {
	// 			log.Fatalf("failed to kill debugger: %v", err)
	// 		}
	// 		log.Logf(0, "debugger is still running...")
	// 	case err := <-done:
	// 		// Debugger hit a breakpoint and thus quit.
	// 		if err != nil {
	// 			log.Fatalf("debugger finished with error = %v", err)
	// 		}
	// 		// The last testcase should be added to the corpus.
	// 		entries := mgr.target.ParseLog(rep.Output)
	// 		if len(entries) > 0 {
	// 			last := entries[len(entries)-1]
	// 			// last.P
	// 			mgr.addInputToCorpus(last.P.Serialize())
	// 			log.Logf(0, "add program to corpus: %s", last.P.Serialize())
	// 		}
	// 		return nil, nil
	// 	}
	// }
...
func (mgr *Manager) newInput(inp rpctype.RPCInput, sign signal.Signal) bool {
	...
	for _, ea := range inp.Cover {
		log.Logf(0, "cov: 0x%x", ea)
	}
	if *flagCorpus {
...
// func (mgr *Manager) addInputToCorpus(prog []byte) bool {
// 	sig := hash.String(prog)
// 	if _, ok := mgr.corpus[sig]; !ok {
// 		mgr.corpusDB.Save(sig, prog, 0)
// 		if err := mgr.corpusDB.Flush(); err != nil {
// 			log.Logf(0, "failed to save corpus database: %v", err)
// 		}
// 	}
