...
func (ctx *Context) dumpCallSignal(coverFile string, info *ipc.HookInfo) {
	if len(info.Signal) == 0 {
		return
	}
	buf := new(bytes.Buffer)
	for _, pc := range info.Signal {
		fmt.Fprintf(buf, "0x%x\n", pc)
	}
	err := osutil.WriteFile(coverFile, buf.Bytes())
	if err != nil {
...
func (ctx *Context) dumpCoverage(coverFile string, info *ipc.ProgInfo) {
	...
	ctx.dumpCallSignal(fmt.Sprintf("%v.hook", coverFile), &info.Hooks)
}
