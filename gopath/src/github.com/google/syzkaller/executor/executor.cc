...
// If true, collect function invocation info
static bool flag_collect_hook;
...
static syz_thread_t* schedule_call(int call_index, int call_num, bool colliding, uint64 copyout_index, uint64 num_args, uint64* args, uint64* pos);
...
static void thread_create(syz_thread_t* th, int id);
...
#if SYZ_EXECUTOR_USES_SHMEM
	// CwT: replace MAP_PRIVATE with MAP_SHARED
	if (mmap(&input_data[0], kMaxInput, PROT_READ, MAP_SHARED | MAP_FIXED, kInFd, 0) != &input_data[0])
...
retry:
	...
	if (flag_collect_hook && !colliding) {
		// Currently we collect coverage for the entire process.
		hook_enable(&hook);
	}
	...
	for (;;) {
		...
		for (uint64 i = num_args; i < kMaxArgs; i++)
			args[i] = 0;
		syz_thread_t* th = schedule_call(call_index++, call_num, colliding, copyout_index,
	...
	if (flag_collect_hook && !colliding && !collide) {
		// Write our result at last
		write_hook_output();
	}
...
syz_thread_t* schedule_call(int call_index, int call_num, bool colliding, uint64 copyout_index, uint64 num_args, uint64* args, uint64* pos)
{
...
void write_hook_output() {
#if SYZ_EXECUTOR_USES_SHMEM
	write_output(0xdeadbeef); // marker
	uint32* hook_count_pos = write_output(0);  // number of pc
	if (!flag_collect_hook) {
		return;
	}
	hook_collect(&hook);
	uint64 *cov = (uint64 *) hook.data;
	// truncate to 32 bits
	for (uint32 i = 0; i < hook.size; i++) {
		// debug("fuzzer cov: 0x%llx\n", cov[i]);
		write_output(cov[i]);
	}
	*hook_count_pos = hook.size;
#endif
}

void thread_create(syz_thread_t* th, int id)
{
	th->created = true;
	th->id = id;
	th->executing = false;
