...
var typeInt = &typeDesc{
	...
	CheckConsts: func(comp *compiler, t *ast.Type, args []*ast.Type, base prog.IntTypeCommon) {
		if len(args) > 0 && len(args[0].Colon) != 0 {
			...
			} else if end-begin > uint64(1<<64-1<<32) {
				comp.error(args[0].Pos, "bad int range [%v:%v] %v", begin, end, uint64(1<<64-1<<32))
				return
			}
...
var typeFlags = &typeDesc{
	...
	Gen: func(comp *compiler, t *ast.Type, args []*ast.Type, base prog.IntTypeCommon) prog.Type {
		...
		// CvvT: For those auto-generated flags, there are always enumeration.
		isbitmask := true
		if matched, _ := regexp.MatchString(`flag_?\d+$`, name); matched {
			isbitmask = false
		}
		...
		return &prog.FlagsType{
			IntTypeCommon: base,
			Vals:          values,
			BitMask:       isbitmask && isBitmask(values),
		}
	},
}
