...
const (
	...
	FlagEnableHook                            // collect function invocation info
)
...
type ProgInfo struct {
	...
	Hooks HookInfo // stores signal from hooker
}
...
func (env *Env) Exec(opts *ExecOpts, p *prog.Prog) (output []byte, info *ProgInfo, hanged bool, err0 error) {
	...
	// FIXME: do we need fallback signal??
	if info != nil && env.config.Flags&FlagSignal == 0 && env.config.Flags&FlagEnableHook == 0 {
...
func (env *Env) parseOutput(p *prog.Prog) (*ProgInfo, error) {
	...
	// Read signal from hooker
	if env.config.Flags&FlagEnableHook != 0 {
		if err := readHooks(&out, &info.Hooks); err != nil {
			return nil, err
		}
	}
...
func readHooks(outp *[]byte, hooks *HookInfo) error {
	// We use a magic number to indicate whether the testcase executed successfully.
	sig, ok := readUint32(outp)
	if !ok {
	...
	size, ok := readUint32(outp)
	if !ok {
		return fmt.Errorf("failed to read size of cov")
	}
	if hooks.Signal, ok = readUint32Array(outp, size); !ok {
		return fmt.Errorf("hooks overflow: %v/%v", size, len(*outp))
	}
	return nil
}
