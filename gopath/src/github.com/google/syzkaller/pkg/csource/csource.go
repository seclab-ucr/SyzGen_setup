...
func (ctx *context) emitCallName(w *bytes.Buffer, call prog.ExecCall, native bool) {
	...
	} else if strings.HasPrefix(callName, "syz_") || callName == "mmap" {
...
func (ctx *context) postProcess(result []byte) []byte {
	...
	if ctx.target.OS == "darwin" {
		result = bytes.Replace(result, []byte("htobe16"), []byte("OSSwapHostToBigInt16"), -1)
		result = bytes.Replace(result, []byte("htole16"), []byte("OSSwapHostToLittleInt16"), -1)
		result = bytes.Replace(result, []byte("be16toh"), []byte("OSSwapBigToHostInt16"), -1)
		result = bytes.Replace(result, []byte("le16toh"), []byte("OSSwapLittleToHostInt16"), -1)

		result = bytes.Replace(result, []byte("htobe32"), []byte("OSSwapHostToBigInt32"), -1)
		result = bytes.Replace(result, []byte("htole32"), []byte("OSSwapHostToLittleInt32"), -1)
		result = bytes.Replace(result, []byte("be32toh"), []byte("OSSwapBigToHostInt32"), -1)
		result = bytes.Replace(result, []byte("le32toh"), []byte("OSSwapLittleToHostInt32"), -1)

		result = bytes.Replace(result, []byte("htobe64"), []byte("OSSwapHostToBigInt64"), -1)
		result = bytes.Replace(result, []byte("htole64"), []byte("OSSwapHostToLittleInt64"), -1)
		result = bytes.Replace(result, []byte("be64toh"), []byte("OSSwapBigToHostInt64"), -1)
		result = bytes.Replace(result, []byte("le64toh"), []byte("OSSwapLittleToHostInt64"), -1)
	}
	return result
}
