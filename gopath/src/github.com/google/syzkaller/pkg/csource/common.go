...
func createCommonHeader(p, mmapProg *prog.Prog, replacements map[string]string, opts Options) ([]byte, error) {
	...
	var args []string
	if sysTarget.CPP == "clang" {
		args = append(args, []string{"-nostdinc", "-undef", "-dDI", "-E", "-P", "-C", "-"}...)
		args = append(args, "-DDARWIN_INCLUDE=#include")
	} else {
		args = append(args, []string{"-nostdinc", "-undef", "-fdirectives-only", "-dDI", "-E", "-P", "-"}...)
	}
	cmd := osutil.Command(sysTarget.CPP, args...)
